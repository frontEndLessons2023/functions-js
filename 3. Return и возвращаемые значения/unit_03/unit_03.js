...
function t3() {
    let ch_3 = document.querySelector('.ch-3');
    return ch_3.checked ? ch_3.value : false;
}
...
function t4() {
    let rb = document.querySelectorAll('.r-4');
...
function t5() {
    ...
    let operation = document.querySelector('.s52').value;
    switch(operation){
...
function t11() {
    ...
    return Array.from(arguments).reduce((res, cur) => res + cur, 0)
}
...
function t12(...rest) {
    ...
    return rest.reduce((res, cur) => res + cur, 0);
}
...
function t26() {
    let res = document.querySelector('.i-26').value;
    res = res.trim();
    if(res.length == 0){
        return;
    }
    document.querySelector('.out-26').innerHTML =  res.toLowerCase();
}
