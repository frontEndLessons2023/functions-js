...
function t6(arr, block) {
    block.innerHTML = arr.reduce((res, cur) => res += cur + ' ', '');
}
...
function t7(arr = [], block) {
    ...
    if(Array.isArray(arr)){
        res = arr.reduce((res, cur) => res += cur + ' ', '');
    }
    block.innerHTML = res;
}
...
function t10() {
    document.querySelector('.out-10').innerHTML = arguments.length;
}
...
function t11() {
    let sum = Array.from(arguments).reduce((res, cur) => res += cur, 0);
    document.querySelector('.out-11').innerHTML = sum;
}
...
function t12(...args) {
    let sum = args.reduce((res, cur) => res += cur, 0);
    document.querySelector('.out-12').innerHTML = sum;
